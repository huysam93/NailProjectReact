...
const HeroSlider = ({ images }) => {
    ...
    if (!images.length) {
        return (
             <div className="relative h-[70vh] bg-cover bg-center text-white flex items-center justify-center" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1522338140262-f462ced66111?q=80&w=2070&auto=format&fit=crop')" }}>
                <div className="absolute inset-0 bg-black bg-opacity-50"></div>
                <p className='z-10'>Loading slider 1...</p>
             </div>
        )
    }
...
const GalleryCarousel = ({ images }) => {
    ...
    return (
        <div className="relative">
            ...
            <button onClick={() => scroll(-1)} className="absolute left-0 top-1/2 -translate-y-1/2 bg-white/50 p-2 rounded-full shadow-md hover:bg-white transition"><ChevronLeft /></button>
            <button onClick={() => scroll(1)} className="absolute right-0 top-1/2 -translate-y-1/2 bg-white/50 p-2 rounded-full shadow-md hover:bg-white transition"><ChevronRight /></button>
        </div>
    );
};
